...
TOOLS = [
    ...
    {"type": "function", "name": "compute", "description": "Derive a number from stored facts. Arithmetic only: + - * / ( ) and fact references like f123. Result is stored as a new fact you can cite. Example: '(f12 - f9) / f9 * 100' with unit '%'.", "parameters": {"type": "object", "properties": {"expression": {"type": "string"}, "metric": {"type": "string", "description": "what the result is, e.g. 'Revenue growth QoQ'"}, "entity": {"type": ["string", "null"]}, "period": {"type": ["string", "null"]}, "unit": {"type": ["string", "null"]}, "decimals": {"type": "integer", "description": "rounding, default 1"}}, "required": ["expression", "metric"]}},
    ...
    {"type": "function", "name": "write_review", "description": "Write the finished document. Every number must be a {{fact:ID}} reference; the renderer fills in stored values and rejects any other digits except years, quarter labels and list numbering. If rejected, fix and call again.", "parameters": {"type": "object", "properties": {"title": {"type": "string"}, "filename": {"type": "string", "description": "e.g. 'Fund III Q2 2026 Portfolio Review.docx'"}, "sections": {"type": "array", "items": {"type": "object", "properties": {"heading": {"type": "string"}, "paragraphs": {"type": "array", "items": {"type": "string"}}, "table": {"type": ["object", "null"], "properties": {"columns": {"type": "array", "items": {"type": "string"}}, "rows": {"type": "array", "items": {"type": "array", "items": {"type": "string"}}}}}}, "required": ["heading"]}}}, "required": ["title", "filename", "sections"]}},
]
